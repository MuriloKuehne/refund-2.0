    ...
    <div>
      ...
      <div className="w-full h-12  flex items-center  rounded-lg border border-gray-300 text-sm text-gray-100 bg-transparent outline-none">
        ...
        <span className="text-xs text-gray-100 flex-1 pl-4">
          {filename ?? "Selecione o arquivo de comprovante"}
        </span>
        <label
          htmlFor="upload"
          className="flex h-12 px-4 items-center bg-green-100 rounded-lg cursor-pointer disabled:opacity-50 hover:bg-green-200 transition ease-in-out"
        >
          <img src={UploadSvg} alt="upload" className="size-6" />
        </label>
      </div>
    </div>
  )
